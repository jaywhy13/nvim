return {
...

	{
		...
		keys = {
			...
			{ "<leader>aicn", "<cmd>CodeCompanionChat<cr>", mode = { "n", "v" }, desc = "Code Companion Chat" },
			{
				"<leader>aica",
								...
								["strategist"] = {
									...
									system_prompt = function(group, ctx)
										return string.format(
											[[You are an elite Technical Strategist.
### YOUR ROLE
...
Use your tools liberally to gather context. Research the web for documentation, grep the codebase for patterns, and read files to understand current implementations. Use 'ask_questions' if the objective is ambiguous.]]
					...
					http = {
						anthropic = function()
							return require("codecompanion.adapters").extend("anthropic", {
								env = {
									api_key = function()
										return vim.fn.system("devx llm-gateway print-token --key"):gsub("%s+$", "")
									end,
								},
	...
	-- Claude code extension
	{
		...
		config = true,
		keys = {
			{ "<leader>aia", nil, desc = "AI Agent" },
			{ "<leader>aiat", "<cmd>ClaudeCode<cr>", desc = "Toggle Claude" },
			{ "<leader>aiaf", "<cmd>ClaudeCodeFocus<cr>", desc = "Focus Claude" },
			{ "<leader>aiar", "<cmd>ClaudeCode --resume<cr>", desc = "Resume Claude" },
			{ "<leader>aiac", "<cmd>ClaudeCode --continue<cr>", desc = "Continue Claude" },
			{ "<leader>aiam", "<cmd>ClaudeCodeSelectModel<cr>", desc = "Select Claude model" },
			{ "<leader>aiab", "<cmd>ClaudeCodeAdd %<cr>", desc = "Add current buffer" },
			{ "<leader>aias", "<cmd>ClaudeCodeSend<cr>", mode = "v", desc = "Send to Claude" },
			{
				"<leader>as",
	...
	-- TTS
	{
		...
		config = function()
			require("tts").setup({
				backend = "openai",
				openai = {
					api_key = vim.fn.system("devx llm-gateway print-token --key"):gsub("%s+$", ""),
					api_url = "https://proxy.shopify.ai/v1/audio/speech",
		...
		keys = {
			{ "<leader>ais", nil, desc = "Speak (TTS)" },
			{ "<leader>aisr", "<cmd>TTS<cr>", desc = "Read out loud", mode = { "v" } },
